...
Usage:
    uv run --no-project scripts/extract_pdf.py <input.pdf> [output.json]
    uv run --no-project scripts/extract_pdf.py --verify <input.pdf]

Examples:
    # Extract and print to stdout
    uv run --no-project scripts/extract_pdf.py profile.pdf

    # Extract with verification summary (recommended)
    uv run --no-project scripts/extract_pdf.py --verify profile.pdf

    # Extract and save to file
    uv run --no-project scripts/extract_pdf.py profile.pdf profile.json
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  uv run --no-project scripts/extract_pdf.py profile.pdf              # Print JSON to stdout
  uv run --no-project scripts/extract_pdf.py --verify profile.pdf     # Print JSON + verification summary
  uv run --no-project scripts/extract_pdf.py profile.pdf out.json     # Save to file
        """,
    )
