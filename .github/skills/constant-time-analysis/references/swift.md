...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --arch x86_64 crypto.swift

# Test multiple optimization levels (RECOMMENDED)
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --opt-level O0 crypto.swift
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --opt-level O2 crypto.swift

# Include conditional branch warnings
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --warnings crypto.swift

# CI-friendly JSON output
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json crypto.swift
```
