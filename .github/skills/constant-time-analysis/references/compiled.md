...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.c

# Go
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.go

# Rust
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.rs
...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --warnings crypto.c

# Filter to specific functions
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --func 'sign|verify|decrypt' crypto.c

# CI-friendly JSON output
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json crypto.c
```
...
- name: Check constant-time properties
  run: |
    uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json src/crypto/*.c
