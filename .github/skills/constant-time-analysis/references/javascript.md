...
```bash
# Analyze JavaScript
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.js

# Analyze TypeScript (transpiles first)
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.ts

# Include warning-level violations
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --warnings crypto.js

# Filter to specific functions
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --func 'encrypt|sign' crypto.js

# JSON output for CI
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json crypto.js
```
