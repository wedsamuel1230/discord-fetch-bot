...
```bash
# Analyze Python file
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.py

# Include warning-level violations
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --warnings crypto.py

# Filter to specific functions
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --func 'encrypt|sign' crypto.py

# JSON output for CI
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json crypto.py
```
