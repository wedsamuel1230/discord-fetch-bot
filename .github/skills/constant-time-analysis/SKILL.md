...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py <source_file>

# Include conditional branch warnings
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --warnings <source_file>

# Filter to specific functions
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --func 'sign|verify' <source_file>

# JSON output for CI
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --json <source_file>
```
...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --arch x86_64 crypto.c
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --arch arm64 crypto.c

# Multiple optimization levels
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --opt-level O0 crypto.c
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --opt-level O3 crypto.c
```
...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py CryptoUtils.java

# Analyze Kotlin bytecode (Android/JVM)
uv run --no-project {baseDir}/ct_analyzer/analyzer.py CryptoUtils.kt

# Analyze C# IL
uv run --no-project {baseDir}/ct_analyzer/analyzer.py CryptoUtils.cs
```
...
uv run --no-project {baseDir}/ct_analyzer/analyzer.py crypto.swift

# Analyze for specific architecture (iOS devices)
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --arch arm64 crypto.swift

# Analyze with different optimization levels
uv run --no-project {baseDir}/ct_analyzer/analyzer.py --opt-level O0 crypto.swift
```
