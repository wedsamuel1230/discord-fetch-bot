...
This guidance follows the repository-wide Memory-Bank policy. For full lifecycle rules and phase workflow (Phase 0–6), see [.github/MEMORY-BANK-PATCH.md](../MEMORY-BANK-PATCH.md).

- Ensure `memory-bank/` exists before work and read files in order: `projectbrief.md`, `activeContext.md`, `SESSION.md`, `README.md`.
- Append session entries to `memory-bank/SESSION.md` in `YYYY-MM-DD — vX.Y.Z` format and write `memory-bank/logs/YYYY-MM-DD.md` on session end.
- Do NOT add `memory-bank/` to `.gitignore`.
```
